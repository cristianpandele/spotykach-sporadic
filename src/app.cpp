...
void AppImpl::drawTestState()
{
    ...
    const uint16_t ring_val_a
        = hw.GetAnalogControlValue((Hardware::AnalogControlId)last_pot_moved_a)
          * Hardware::kNumLedsPerRing;
    const uint16_t ring_val_b
        = hw.GetAnalogControlValue((Hardware::AnalogControlId)last_pot_moved_b)
          * Hardware::kNumLedsPerRing;
    ...
    // Spotykach Slider
    float skval
        = (hw.GetAnalogControlValue(Hardware::CTRL_SPOTYKACH) * 2.0f - 1.0f)
          + hw.GetControlVoltageValue(Hardware::CV_SPOTYKACH);
    skval = daisysp::fclamp(skval, -1.0f, 1.0f);
    hw.leds.Set(
        Hardware::LED_SPOTY_SLIDER_B, 0xff0000, daisysp::fmax(skval, 0.0f));
    hw.leds.Set(
        Hardware::LED_SPOTY_SLIDER_A, 0x0000ff, daisysp::fmax(-skval, 0.0f));
    ...
    hw.leds.Set(
        Hardware::LED_DRIFT_B, cv_b >= 0.0f ? 0xff0000 : 0x0000ff, fabsf(cv_b));
...
void AppImpl::testSDCard()
{
    ...
    uint32_t offset = 0;
    Log::PrintLine(
        "Writing ~11.5MB (approx 60s stereo 16-bit audio) to file...");
    ...
    Log::PrintLine(
        "Reading ~11.5MB (approx 60s stereo 16-bit audio) from file...");
